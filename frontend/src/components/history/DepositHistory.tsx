...
export default function DepositHistory({ transactions }: HistoryProps) {
  ...
  const getStatusDisplay = (status: string) => {
    ...
    if (s === "SWEPT" || s === "CREDITED" || s === "COMPLETED") return { label: "COMPLETED", color: "bg-[#0f2a1d] text-[#39ff88]" };
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`text-[11px] font-black px-2 py-0.5 rounded uppercase ${getStatusDisplay(selectedTx.status === "SWEPT" ? "CREDITED" : selectedTx.status).color}`}>
                  {getStatusDisplay(selectedTx.status === "SWEPT" ? "CREDITED" : selectedTx.status).label}
                </span>
              </div>
              <div className="w-full h-px bg-[#1a1f28]/50" />
